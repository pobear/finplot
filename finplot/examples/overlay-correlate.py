...
daily_plot.colors.update(
    dict(bull_body="#bfb", bull_shadow="#ada", bear_body="#fbc", bear_shadow="#dab")
)
daily_plot.x_offset = (
    3.1  # resample() gets us start of day, offset +1.1 (gap+off center wick)
)
...
errfun = lambda arr: [
    y - arr[0] * x + arr[1] for x, y in zip(dfc.ret_alphabet, dfc.ret_microsoft)
]
