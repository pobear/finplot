...
fplt.candle_bull_color = (
    fplt.volume_bull_color
) = fplt.candle_bull_body_color = fplt.volume_bull_body_color = "#352"
...
plot = fplt.renko(
    df[["Close", "Volume"]], ax=ax
)  # let renko transform volume by passing it in as an extra column
df_renko = (
    plot.datasrc.df
)  # names of new renko columns are time, open, close, high, low and whatever extras you pass in
