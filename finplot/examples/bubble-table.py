...
def resample(prices, quotes):
    quotes.bidPrice = (quotes.bidPrice * quotes.bidSize).rolling(
        downsample
    ).sum() / quotes.bidSize.rolling(downsample).sum()
    quotes.bidSize = quotes.bidSize.rolling(downsample).sum()
    quotes.askPrice = (quotes.askPrice * quotes.askSize).rolling(
        downsample
    ).sum() / quotes.askSize.rolling(downsample).sum()
...
def plot_bubble_pass(price, price_col, size_col, min_val, max_val, scale, color, ax):
    price = price.copy()
    price.loc[
        (price[size_col] < min_val) | (price[size_col] > max_val), price_col
    ] = np.nan
...
def plot_quote_bubbles(quotes, ax):
    ...
    for a, b in reversed(rng):
        ...
        plot_bubble_pass(
            quotes, "bidPrice", "bidSize2", a, b, scale=scale, color="#0f0", ax=ax
        )
        plot_bubble_pass(
            quotes, "askPrice", "askSize2", a, b, scale=scale, color="#f00", ax=ax
        )
...
def plot_quote_table(quotes, ax):
    ...
    def skip_y_crosshair_info(
        x, y, xt, yt
    ):  # we don't want any Y crosshair info on the table
        return xt, ""
    ...
    colmap = fplt.ColorMap(
        [0.0, 0.5, 1.0], [[200, 80, 60], [200, 190, 100], [40, 170, 30]]
    )  # traffic light colors
    fplt.heatmap(
        quotes[[1, 0]], colmap=colmap, colcurve=lambda x: x, ax=ax
    )  # linear color mapping
    fplt.labels(
        ts,
        [1.5] * count,
        ["%.1f" % (v / 1e6) for v in quotes["askSize"]],
        ax=ax2,
        anchor=(0.5, 0.5),
    )
    fplt.labels(
        ts,
        [0.5] * count,
        ["%.1f" % (v / 1e6) for v in quotes["bidSize"]],
        ax=ax2,
        anchor=(0.5, 0.5),
    )
...
ax, ax2 = fplt.create_plot(
    f"BitMEX {downsample}m quote bubble plot + quote table", rows=2, maximize=False
)
fplt.windows[0].ci.layout.setRowStretchFactor(
    0, 10
)  # make primary plot large, and implicitly table small
