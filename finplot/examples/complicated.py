...
class BinanceFutureWebsocket:
    ...
    def subscribe(self, symbol, interval):
        try:
            data = '{"method":"SUBSCRIBE","params":["%s@kline_%s"],"id":1}' % (
                symbol,
                interval,
            )
            self.ws.send(data)
...
def do_load_price_history(symbol, interval):
    url = "https://www.binance.com/fapi/v1/klines?symbol=%s&interval=%s&limit=%s" % (
        symbol,
        interval,
        1000,
    )
...
def calc_plot_data(df, indicators):
    ...
    last_col = (
        ...
        if last_close > price.iloc[-2].Close
        else fplt.candle_bear_color
    )
...
def change_asset(*args, **kwargs):
    ...
    if data["rsi"] is not None:
        ...
        plots["sar"] = fplt.plot(
            data["sar"], color="#55a", style="+", width=0.6, legend="SAR", ax=ax
        )
        plots["rsi"] = fplt.plot(data["rsi"], legend="RSI", ax=ax_rsi)
        plots["stoch"] = fplt.plot(
            data["stoch"], color="#880", legend="Stoch", ax=ax_rsi
        )
...
def dark_mode_toggle(dark):
    ...
    fplt.volume_bull_color = fplt.volume_bull_body_color = (
        fplt.candle_bull_color + volume_transparency
    )
        ...
        for item in ax.items:
            if isinstance(item, fplt.FinPlotItem):
                ...
                if not isvolume:
                    item.colors.update(
                        dict(
                            bull_shadow=fplt.candle_bull_color,
                    ...
                    item.colors.update(
                        dict(
                            bull_frame=fplt.volume_bull_color,
...
ax, ax_rsi = fplt.create_plot(
    "Complicated Binance Futures Example", rows=2, init_zoom_periods=300
)
axo = ax.overlay()
