...
utc2timestamp = lambda s: int(
    dateutil.parser.parse(s).replace(tzinfo=pytz.utc).timestamp() * 1000
)


def download_price_history(
    symbol="BTCUSDT", start_time="2020-06-22", end_time="2020-08-19", interval_mins=1
):
    interval_ms = 1000 * 60 * interval_mins
    interval_str = (
        "%sm" % interval_mins if interval_mins < 60 else "%sh" % (interval_mins // 60)
    )
    ...
    for start_t in range(start_time, end_time, 1000 * interval_ms):
        ...
        url = (
            "https://www.binance.com/fapi/v1/klines?interval=%s&limit=%s&symbol=%s&startTime=%s&endTime=%s"
...
def calc_volume_profile(df, period, bins):
    ...
    ) / 3  # assume this is volume center per each 1m candle
...
time_volume_profile = calc_volume_profile(
    df, period="W", bins=100
)  # try fewer/more horizontal bars (graphical resolution only)
