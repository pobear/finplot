...
def price_history(load_periods=500, interval_mins=1):
    ...
    url = baseurl + "/udf/history?symbol=%s&resolution=%s&from=%s&to=%s" % (
        symbol,
        interval_mins,
        start_t,
        end_t,
...
def update_plot():
    ...
    if not plots:  # 1st time
        ...
        orderbook_plot = fplt.horiz_time_volume(
            orderbook, candle_width=1, draw_body=10, colorfunc=orderbook_colorfunc
        )
...
if __name__ == "__main__":
    ...
    thread.start()
    fplt.create_plot(
        "Realtime Bitcoin/Dollar 1m (BitMEX websocket)",
