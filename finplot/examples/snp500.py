...
interval = "1d"
url = (
    "https://query1.finance.yahoo.com/v7/finance/download/%s?period1=%s&period2=%s&interval=%s&events=history"
    % (symbol, start_t, end_t, interval)
)
r = requests.get(url, headers={"user-agent": "Mozilla/5.0"})
df = pd.read_csv(StringIO(r.text))
df["Date"] = pd.to_datetime(df["Date"]).view(
    "int64"
)  # use finplot's internal representation, which is ns
...
def update_legend_text(x, y):
    ...
    fmt = '<span style="color:#%s">%%.2f</span>' % (
        "0b0" if (row.Open < row.Close).all() else "a00"
    )
    ...
    hover_label.setText(
        rawtxt % (symbol, interval.upper(), row.Open, row.Close, row.High, row.Low)
    )
