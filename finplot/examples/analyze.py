...
def download_price_history(
    symbol="XBTUSD", start_time="2020-01-10", end_time="2020-05-07", interval_mins=60
):
    ...
    for start_t in range(start_time, end_time, 10000 * 60 * interval_mins):
        ...
        if end_t > end_time:
            end_t = end_time
        url = "%s/udf/history?symbol=%s&resolution=%s&from=%s&to=%s" % (
            baseurl,
            symbol,
            interval_mins,
            start_t,
...
def plot_rsi(df, ax):
    ...
    with np.errstate(invalid="ignore"):
        ...
        losses[
            (losses <= 0) | np.isnan(losses)
        ] = 1e-10  # we don't want divide by zero/NaN
    n = 14
    m = (n - 1) / n
    ni = 1 / n
