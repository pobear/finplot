...
log_plot_offset = (
    -2.2222222e-16
)  # I could file a bug report, probably in PyQt, but this is more fun
...
class EpochAxisItem(pg.AxisItem):
    ...
    def tickStrings(self, values, scale, spacing):
        ...
        if all(
            s.endswith(" 00:00") for s in strs if s
...
class PandasDataSource:
    ...
    def __init__(self, df):
        if (
            ...
            or df.index[-1] > 1e7
            or ".RangeIndex" not in str(type(df.index))
        ):
            df = df.reset_index()
        self.df = df.copy()
        # manage time column
        if _has_timecol(self.df):
            ...
            self.col_data_offset = (
                1  # no. of preceeding columns for other plots and time column
            )
        else:
            self.standalone = False
            self.col_data_offset = (
                0  # no. of preceeding columns for other plots and time column
            )
    ...
    def addcols(self, datasrc):
        new_scale_cols = [
            c + len(self.df.columns) - datasrc.col_data_offset
            for c in datasrc.scale_cols
        ]
        ...
        if _has_timecol(datasrc.df):
            ...
            timecol = (
                timecol if timecol in datasrc.df.columns else datasrc.df.columns[0]
            )
        ...
        self.is_sparse = (
            self.is_sparse
            or self.df[self.df.columns[self.col_data_offset]].isnull().sum().max()
            > ldf2
        )
        datasrc.is_sparse = (
            datasrc.is_sparse
            or datasrc.df[datasrc.df.columns[datasrc.col_data_offset]]
            .isnull()
            .sum()
            .max()
            > ldf2
    ...
    def update(self, datasrc):
        ...
        input_df = pd.merge(
            input_df, df[[]], how="outer", left_index=True, right_index=True
        )
...
class FinCrossHair:
    ...
    def update(self, point=None):
        ...
        sd, se = (
            (self.ax.significant_decimals, self.ax.significant_eps)
            if clamp_grid
            else (significant_decimals, significant_eps)
        )
        timebased = False
...
class FinViewBox(pg.ViewBox):
    ...
    def mouseLeftDrag(self, ev, axis):
        ...
        if not self.drawing:
            ...
            self.draw_line = FinPolyLine(
                self,
                [p0, p1],
                closed=False,
                pen=pg.mkPen(draw_line_color),
                movable=False,
            )
    ...
    def mouseMiddleDrag(self, ev, axis):
        ...
        if not self.drawing:
            ...
            self.draw_ellipse = FinEllipse(
                p0, s, pen=pg.mkPen(draw_line_color), movable=True
            )
    ...
    def linkedViewChanged(self, view, axis):
        ...
        if view and self.datasrc and view.datasrc:
            ...
            if is_same_scale:  # stable zoom based on index
                if (
                    is_dirty
                    or abs(vr.left() - tr.left()) >= 1
                    or abs(vr.right() - tr.right()) >= 1
                ):
                    if is_dirty:
                ...
                if is_dirty or abs(vt0 - tt0) >= period2 or abs(vt1 - tt1) >= period2:
                    ...
                    if self.parent():
                        x0, x1 = _pdtime2index(
                            self.parent(), pd.Series([vt0, vt1]), any_end=True
                        )
    ...
    def refresh_all_y_zoom(self):
        ...
        main_vb.force_range_update = (
            len(self.win.axs) - 1
        )  # update main as many times as there are other rows
        self.update_y_zoom()
    ...
    def update_y_zoom(self, x0=None, x1=None):
        ...
        xr = min(
            _round(x1 - side_margin) + side_margin,
            datasrc.xlen + right_margin_candles - side_margin,
        )
        dxl = xl - x0
        dxr = xr - x1
        if dxl > 0:
        ...
        if self.yscale.scaletype == "log":
            if lo < 0:
                lo = (
                    0.05 * self.yscale.scalef
                )  # strange QT log scale rendering, which I'm unable to compensate for
            else:
                lo = max(1e-100, lo)
            ...
            rng = max(
                rng, 2e-7
            )  # some very weird bug where high/low exponents stops rendering
            base = (hi + lo) * self.v_zoom_baseline
            y0 = base - rng * self.v_zoom_baseline
            y1 = base + rng * (1 - self.v_zoom_baseline)
...
class FinPlotItem(pg.GraphicsObject):
    ...
    def update_dirty_picture(self, visibleRect):
        if self.dirty or (
            self.lod
            and (  # regenerate when zoom changes?
                visibleRect.left() < self.cachedRect.left()
                or visibleRect.right() > self.cachedRect.right()
                or visibleRect.width() < self.cachedRect.width() / cache_candle_factor
            )
    ...
    def _generate_picture(self, boundingRect):
        ...
        self.cachedRect = QtCore.QRectF(
            boundingRect.left() - (cache_candle_factor - 1) * 0.5 * w,
            0,
            cache_candle_factor * w,
            0,
        )
    ...
    def _generate_dummy_picture(self, boundingRect):
        if self.datasrc.is_sparse:
            ...
            self.painter.drawRect(
                QtCore.QRectF(l, boundingRect.top(), 1e-3, boundingRect.height() * 1e-5)
            )
            self.painter.drawRect(
                QtCore.QRectF(
                    r, boundingRect.bottom(), -1e-3, -boundingRect.height() * 1e-5
                )
            )
...
class CandlestickItem(FinPlotItem):
    ...
    def generate_picture(self, boundingRect):
        ...
        for shadow, frame, body, df_rows in self.colorfunc(self, self.datasrc, df):
            ...
            if (
                self.draw_body and not drawing_many_shadows
            ):  # settle with only drawing shadows if too much detail
    ...
    def rowcolors(self, prefix):
        return [
            self.colors[prefix + "_shadow"],
            self.colors[prefix + "_frame"],
            self.colors[prefix + "_body"],
        ]
...
class HeatmapItem(FinPlotItem):
    ...
    def generate_picture(self, boundingRect):
        ...
        values = values / (
            np.nanmax(values) / (1 + self.whiteout)
        )  # overshoot for coloring
...
class HorizontalTimeVolumeItem(CandlestickItem):
    def __init__(
        ...
        ## self.col_data_end = len(datasrc.df.columns)
        colorfunc = (
            colorfunc or horizvol_colorfilter()
        )  # resolve function lower down in source code
        super().__init__(
            ax,
            datasrc,
            draw_shadow=False,
        ...
        self.colors.update(
            dict(
                neutral_shadow=volume_neutral_color,
    ...
    def generate_picture(self, boundingRect):
        ...
        for i in range(len(prices)):
            ...
            if draw_body:
                ...
                for shadow, frame, body, data in self.colorfunc(
                    self, self.datasrc, np.array([prcr, volr])
                ):
                    p.setPen(pg.mkPen(frame))
                    p.setBrush(pg.mkBrush(body))
                    prcr_, volr_ = data
            ...
            if self.draw_poc:
                ...
                p.drawLine(
                    QtCore.QPointF(t, y), QtCore.QPointF(t + f * self.draw_poc, y)
                )
...
def create_plot(
    ...
    ax0 = axs = create_plot_widget(
        master=win, rows=rows, init_zoom_periods=init_zoom_periods, yscale=yscale
    )
...
def create_plot_widget(master, rows=1, init_zoom_periods=1e10, yscale="linear"):
    ...
    for n in range(rows):
        ...
        viewbox = FinViewBox(
            master,
            init_steps=init_zoom_periods,
            yscale=ysc,
            v_zoom_scale=1 - y_pad,
            enableMenu=False,
        )
        ax = prev_ax = _add_timestamp_plot(
            master=master, prev_ax=prev_ax, viewbox=viewbox, index=n, yscale=ysc
        )
        if axs:
    ...
    if isinstance(master, pg.GraphicsLayoutWidget):
        proxy = pg.SignalProxy(
            ...
            rateLimit=144,
            slot=partial(_mouse_moved, master),
        )
    else:
        proxy = []
        for ax in axs:
...
def renko(x, y=None, bins=None, step=None, ax=None, colorfunc=price_colorfilter):
    ...
    adj = (
        ...
        else _adjust_renko_datasrc
    )
    step_adjust_renko_datasrc = partial(adj, bins, step)
    step_adjust_renko_datasrc(datasrc)
    ax.decouple()
    item = candlestick_ochl(
        datasrc, draw_shadow=False, candle_width=1, ax=ax, colorfunc=colorfunc
    )
...
def horiz_time_volume(datasrc, ax=None, **kwargs):
    ...
    if ax.vb.datasrc is not None:
        datasrc.standalone = (
            True  # only force standalone if there is something on our charts already
        )
    datasrc.scale_cols = [
        datasrc.col_data_offset,
        len(datasrc.df.columns) - 2,
    ]  # first and last price columns
    datasrc.pre_update = lambda df: df.loc[
        :, : df.columns[0]
    ...
    item.update_data = partial(
        _update_data, _preadjust_horiz_datasrc, _adjust_horiz_datasrc, item
    )
    item.update_gfx = partial(_update_gfx, item)
    item.setZValue(-10)
    ax.addItem(item)
...
def bar(x, y=None, width=0.8, ax=None, colorfunc=strength_colorfilter, **kwargs):
    ...
    datasrc = _create_datasrc(ax, x, y)
    _adjust_bar_datasrc(
        datasrc, order_cols=False
    )  # don't rearrange columns, done for us in volume_ocv()
...
def hist(x, bins, ax=None, **kwargs):
    ...
    data = [
        (i.mid, 0, hist_data.loc[i], hist_data.loc[i]) for i in sorted(hist_data.index)
    ]
...
def plot(
    ...
    **kwargs
    ...
    if style is None or any(ch in style for ch in "-_."):
        ...
        symbol = {"v": "t", "^": "t1", ">": "t2", "<": "t3"}.get(
            style, style
        )  # translate some similar styles
        yfilter = y.notnull()
        ser = y.loc[yfilter]
...
def add_line(
    p0, p1, color=draw_line_color, width=1, style=None, interactive=False, ax=None
):
    ax = _create_plot(ax=ax, maximize=False)
    used_color = _get_color(ax, style, color)
    pen = _makepen(color=used_color, style=style, width=width)
...
def set_time_inspector(inspector, ax=None, when="click"):
    ...
    if when == "hover":
        master.proxy_hover = pg.SignalProxy(
            master.scene().sigMouseMoved,
            rateLimit=15,
            slot=partial(_inspect_pos, ax, inspector),
        )
    elif when in ("dclick", "double-click"):
        master.proxy_dclick = pg.SignalProxy(
            master.scene().sigMouseClicked,
            slot=partial(_inspect_clicked, ax, inspector, True),
        )
    else:
        master.proxy_click = pg.SignalProxy(
            master.scene().sigMouseClicked,
            slot=partial(_inspect_clicked, ax, inspector, False),
        )
...
def refresh():
    for win in windows:
        vbs = [ax.vb for ax in win.axs] + [
            ax.vb for ax in overlay_axs if ax.vb.win == win
        ]
        for vb in vbs:
            vb.pre_process_data()
        if viewrestore:
        ...
        for vb in vbs:
            ...
            if datasrc and (
                vb.linkedView(0) is None
                or vb.linkedView(0).datasrc is None
                or vb.master_viewbox
            ):
...
def _loadwindata(win):
    ...
    for vb in vbs:
        ...
        if ds and (
            ...
            if (
                kvs["min_x"] >= ds.x.iloc[0] - period_ns
                and kvs["max_x"] <= ds.x.iloc[-1] + period_ns
            ):
                x0, x1 = (
                    ds.x.loc[ds.x >= kvs["min_x"]].index[0],
                    ds.x.loc[ds.x <= kvs["max_x"]].index[-1],
                )
                if x1 == len(ds.x) - 1:
    ...
    try:
        ...
        for ax in win.axs:
            ...
            if ax.vb.datasrc is None:
                continue
            t0, t1, _, _, _ = ax.vb.datasrc.hilo(
                ax.vb.targetRect().left(), ax.vb.targetRect().right()
            )
...
def _add_timestamp_plot(master, prev_ax, viewbox, index, yscale):
    ...
    axes = {
        "bottom": _create_axis(pos="x", vb=viewbox, orientation="bottom"),
    ...
    if y_label_width:
        ax.axes["right"]["item"].setWidth(
            y_label_width
        )  # this is to put all graphs on equal footing when texts vary from 0.4 to 2000000
    ax.axes["right"]["item"].setStyle(
        tickLength=-5
    )  # some bug, totally unexplicable (why setting the default value again would fix repaint width as axis scale down)
...
def _create_legend(ax):
    ...
    if ax.legend is None:
        ax.legend = FinLegendItem(
            border_color=legend_border_color,
            fill_color=legend_fill_color,
            size=None,
            offset=(3, 2),
        )
    ...
    if len(datasrc.df.columns) == 1:
        ...
        for a in ax.vb.win.axs:
            if a.vb.datasrc and len(a.vb.datasrc.df.columns) >= 2:
                datasrc.df.columns = a.vb.datasrc.df.columns[
                    1 : len(datasrc.df.columns) + 1
                ]
        ...
        len(iargs) >= 2
        and len(datasrc.df.columns) == len(iargs) + 1
        and len(iargs) == len(args)
    ):
        try:
            if ".Int" in str(type(iargs[0].index)):
                print(
                    "WARNING: performance penalty and crash may occur when using int64 instead of range indices."
                )
...
def _set_datasrc(ax, datasrc, addcols=True):
    ...
    if not datasrc.standalone:
        if viewbox.datasrc is None:
            ...
            indices_updated = (
                viewbox.datasrc.timebased() and t0 != viewbox.datasrc.x.loc[0]
            )
            for item in ax.items:
                if hasattr(item, "datasrc") and not item.datasrc.standalone:
                    item.datasrc.set_df(
                        viewbox.datasrc.df
                    )  # every plot here now has the same time-frame
                    if indices_updated:
...
def _adjust_renko_datasrc(bins, step, datasrc):
    ...
    bricks = (
        (datasrc.y[bricks.isnull() | (bricks.abs() >= 0.5)] / step).round().astype(int)
    )
    ...
    datasrc.set_df(
        pd.DataFrame(
            data, columns="time open close high low".split() + list(extras.columns)
        )
    )
...
def _adjust_volume_datasrc(datasrc):
    if len(datasrc.df.columns) <= 4:
        datasrc.df.insert(
            3, "_zero_", [0] * len(datasrc.df)
        )  # base of candles is always zero
    datasrc.set_df(
        datasrc.df.iloc[:, [0, 3, 4, 1, 2]]
    )  # re-arrange columns for rendering
...
def _adjust_bar_datasrc(datasrc, order_cols=True):
    ...
    if len(datasrc.df.columns) <= 4:
        ...
        datasrc.df.insert(
            2, "_close_", datasrc.df.iloc[:, 3]
        )  # "close" (actual bar value) for color
    if order_cols:
        datasrc.set_df(
            datasrc.df.iloc[:, [0, 3, 4, 1, 2]]
        )  # re-arrange columns for rendering
...
def _update_data(preadjustfunc, adjustfunc, item, ds, gfx=True):
    ...
    if adjustfunc:
        adjustfunc(ds)
    cs = list(item.datasrc.df.columns[:1]) + list(
        item.datasrc.df.columns[item.datasrc.col_data_offset :]
    )
...
def _update_gfx(item):
    ...
    for i in item.ax.items:
        ...
        li = len(i.datasrc.df)
        if (
            lc and li and max(lc, li) / min(lc, li) > 100
        ):  # TODO: should be typed instead
            continue
...
def _xminmax(datasrc, x_indexed, init_steps=None, extra_margin=0):
    if x_indexed and init_steps:
        ...
        x1 = (
            datasrc.xlen + right_margin_candles - 1 + side_margin + extra_margin
        )  # add another margin to get the "snap back" sensation
    else:
...
def _paint_scatter(item, p, *args):
    with np.errstate(
        invalid="ignore"
    ):  # make pg's mask creation calls to numpy shut up
...
def _get_color(ax, style, wanted_color):
    ...
    if index is None:
        avoid = set(
            ...
            and i.opts["handed_color"] is not None
        ...
        index = len(
            [
                ...
                for i in ax.items
                if isinstance(i, pg.PlotDataItem)
                and i.opts["handed_color"] is None
                and this_line == is_line(i.opts["symbol"])
            ]
        )
...
def _get_datasrc(ax, require=True):
    ...
    if require:
        assert (
            ax.vb.datasrc
        ), "not possible to plot this primitive without a prior time-range to compare to"
...
def _x2local_t(datasrc, x):
    ...
    return _x2t(
        ...
        lambda t: _millisecond_tz_wrap(
            datetime.fromtimestamp(t / 1e9, tz=display_timezone).isoformat(sep=" ")
        ),
    )
...
def _x2utc(datasrc, x):
    ...
    return _x2t(
        datasrc,
        x,
        lambda t: pd.to_datetime(t, unit="ns").strftime("%Y-%m-%d %H:%M:%S.%f"),
    )
...
def _round_to_significant(rng, rngmax, x, significant_decimals, significant_eps):
    is_highres = (
        (rng / significant_eps > 1e2 and rngmax < 1e-2)
        or abs(rngmax) > 1e7
        or rng < 1e-5
    )
...
def _roihandle_move_snap(
    vb, orig_func, pos, modifiers=QtCore.Qt.KeyboardModifier(), finish=True
):
...
try:
    ...
    if qtver not in ("5.9", "5.13") and [int(i) for i in pg.__version__.split(".")] <= [
        0,
        11,
        0,
    ]:
        print(
            "WARNING: your version of Qt may not plot curves containing NaNs and is not recommended."
        )
...
if (
    code is not None
    and any(sanctioned in code.lower() for sanctioned in "_ru _by ru_ be_".split())
    or any(sanctioned in code.lower() for sanctioned in "ru be".split())
):
    import os
...
pg.widgets.GraphicsView.GraphicsView.wheelEvent = partialmethod(
    _wheel_event_wrapper, pg.widgets.GraphicsView.GraphicsView.wheelEvent
)
...
try:
    ...
    candle_shadow_width = int(
        user32.GetSystemMetrics(0) // 2100 + 1
